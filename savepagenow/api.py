...
def advanced_request_status(accesskey: str, secretkey: str, job_ids: List[str]) -> Dict[str, Dict[str, str]]:
    url = BASE_URL
...
def save_page(
    ...
    if if_not_archived_within_outlink and not if_not_archived_within:
        raise ValueError("if_not_archived_within_outlink cannot be given without if_not_archived_within.")
    ...
    response = requests.request("POST", save_url, headers=headers, data=payload_str)
    return response
